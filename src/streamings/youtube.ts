import { parse, SubtitleItem } from "subtitle"; // Importing parse function and SubtitleItem type from "subtitle"
...
class Youtube implements Service {
  ...
  public async getSubs(label: string, highlightIndex?: number): Promise<SubtitleItem[]> { // Added optional parameter for highlightIndex
    ...
    if (highlightIndex !== undefined && highlightIndex >= 0 && highlightIndex < subs.length) { // Highlight logic
      subs.forEach((subtitle, index) => {
        subtitle.data = subtitle.data.replace(/<\/?[^>]+(>|$)/g, ""); // Strip HTML tags
        if (index === highlightIndex) {
          subtitle.data = `<span class="highlight">${subtitle.data}</span>`; // Wrap highlighted text in <span>
        }
      });
    }
